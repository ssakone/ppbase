...
The max returned collections per page (_default to 30_).
...
If _true_ all existing collections and schema fields that are not present in the imported configuration **will be deleted**, including their related records data (default to _false_).
...
Returns an object with all of the collection types and their default fields _(used primarily in the Dashboard UI)_.
