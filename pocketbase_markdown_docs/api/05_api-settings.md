...
├─ Optional _hideControls_

Boolean

Hides the collection create and update controls from the Dashboard. Useful to prevent making accidental schema changes when in production environment.

├─ Required _senderName_

String

Transactional mails sender name.

├─ Required _senderAddress_
...
Max retention period. Set to _0_ for no logs.
...
When _false_ _StartTLS_ command is send, leaving the server to decide whether to upgrade the connection or not).

├─ Optional _authMethod_

String

The SMTP AUTH method to use - _PLAIN_ or _LOGIN_ (used mainly by Microsoft).
Default to _PLAIN_ if empty.

└─ Optional _localName_
...
└─ Optional _forcePathStyle_

Boolean

Forces the S3 request to use path-style addressing, e.g. "https://s3.amazonaws.com/BUCKET/KEY" instead of the default "https://BUCKET.s3.amazonaws.com/KEY".
...
The name or id of the auth collection. Fallbacks to _\_superusers_ if not set.
