...
export const FIELD_TYPE_CONFIG: Record<string, FieldTypeConfig> = {
  ...
  bool:     { label: 'Bool',        icon: '◉',  bg: '#ecfdf5', color: '#059669', badgeClass: 'bg-green-100 text-green-700' },
  ...
  date:     { label: 'Datetime',    icon: '⊞',  bg: '#fffbeb', color: '#d97706', badgeClass: 'bg-amber-100 text-amber-700' },
  ...
  // System / special field types
  autoId:   { label: 'ID',          icon: '~>', bg: '#f0f0f0', color: '#64748b', badgeClass: 'bg-slate-100 text-slate-600' },
  autodate: { label: 'Autodate',    icon: '⊞',  bg: '#f5f3ff', color: '#7c3aed', badgeClass: 'bg-purple-100 text-purple-600' },
}

/** Return the field type config key for a system column. */
export function getSystemColumnType(name: string): string {
  if (name === 'id') return 'autoId'
  if (name === 'created' || name === 'updated') return 'autodate'
  return 'text'
}
